...
def group_prize_ev_func(board, is_black_turn):
    h = basic_ev_func(board, True)
    for row in range(BOARD_HEIGHT):
        for col in range(BOARD_WIDTH):
            if board.board[row][col].is_black():
                if row > 0 and col > 0 and board.board[row-1][col-1].is_black():
                    h -= 0.25
                if row > 0 and col + 1 < BOARD_WIDTH and board.board[row-1][col+1].is_black():
                    h -= 0.25
                if row + 1 < BOARD_HEIGHT and col > 0 and board.board[row+1][col-1].is_black():
                    h -= 0.25
                if row + 1 < BOARD_HEIGHT and col + 1 < BOARD_WIDTH and board.board[row+1][col-1].is_black():
                    h -= 0.25
            ...
            if board.board[row][col].is_white():
                if row > 0 and col > 0 and board.board[row-1][col-1].is_white():
                    h += 0.25
                if row > 0 and col + 1 < BOARD_WIDTH and board.board[row-1][col+1].is_white():
                    h += 0.25
                if row + 1 < BOARD_HEIGHT and col > 0 and board.board[row+1][col-1].is_white():
                    h += 0.25
                if row + 1 < BOARD_HEIGHT and col + 1 < BOARD_WIDTH and board.board[row+1][col-1].is_white():
                    h += 0.25
                if row == 0 or row == BOARD_HEIGHT:
                    h += 0.5
    return h
...
def minimax_a_b(board, depth, plays_as_black, ev_func):
    ...
    for possible_move in possible_moves:
        ...
        ev = minimax_a_b_recurr(new_board, depth-1, (not plays_as_black), a, b, ev_func)
...
def minimax_a_b_recurr(board, depth, black_turn, a, b, ev_func):

    if depth == 0:
        return ev_func(board, black_turn)

    possible_moves = board.get_possible_moves(black_turn)
    ...
    if black_turn:
        ...
        for move in possible_moves:
            ...
            ev = minimax_a_b_recurr(new_board, depth-1, False, alpha, beta, ev_func)
        ...
        for move in possible_moves:
            new_board = deepcopy(board)
            new_board.make_move(move)
            ev = minimax_a_b_recurr(new_board, depth-1, True, alpha, beta, ev_func)
...
class Board:
    ...
    def end(self):
        ...
        if self.black_repeats and self.white_repeats:
            ...
            if ev<0:                                        #changed from ev>0 to ev<0
                self.black_won=True
            elif ev>0:                                      #changed from ev<0 to ev>0
                self.white_won=True
            else:
                self.black_won=True
                self.white_won=True
...
def ai_vs_ai(depth, tactic_variant):
    board = Board()
    is_running = True

    while is_running:
        if board.white_turn:
            move = minimax_a_b( board, MINIMAX_DEPTH, False, basic_ev_func)
        else:
            if tactic_variant == 0:
                move = minimax_a_b( board, depth, True, basic_ev_func)
            if tactic_variant == 1:
                move = minimax_a_b( board, depth, True, push_forward_ev_func)
            if tactic_variant == 2:
                move = minimax_a_b( board, depth, True, push_to_opp_half_ev_func)
            if tactic_variant == 3:
                move = minimax_a_b( board, depth, True, group_prize_ev_func)
    ...
    print("depth: ", depth)
    print("tactic variant: ", tactic_variant)
    #if both won then it is a draw!


main()
# for depth in range(2, 8):
#     for tactic_variant in range(0,4):
#         ai_vs_ai(depth, tactic_variant)